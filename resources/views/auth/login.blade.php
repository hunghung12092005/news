...
<div class="container" id="container">
    <div class="form-container sign-up">
        <form method="POST" action="{{ route('login.register') }}">
            ...
            <?php endif; ?>
            <?php if (!empty($error['existUser'])): ?>
            <span class="error3"><?= implode(',', $error['existUser']) ?></span>
            <?php endif; ?>
            <input type="email" placeholder="Email" name="email">
            <input type="password" placeholder="Password" name="password">
             <?php if (!empty($error['confirm'])): ?>
            <span class="error"><?= implode(',', $error['confirm']) ?></span>
            <?php endif; ?>
            <input type="password" placeholder="Password Confirm" name="password_confirmation"> 
            <button name="CreateAccount">Sign Up</button>
        </form>
    </div>
    <div class="form-container sign-in">
        <form method="POST" action="{{ route('login.login') }}" id="test">
            @csrf
            <h1>Sign In</h1>
            @if ($errors->any())
            ...
            <?php if (!empty($error['userFind'])): ?>
            <span class="error2"><?= implode(',', $error['userFind']) ?></span>
            <?php endif; ?>
            <input type="password" placeholder="Password" name="password">
...
<script>
    ...
    //Xuử lý khi có lỗi lúc đăng nhập
    if (error2) {
        container.classList.remove("active");
    }
</script>
